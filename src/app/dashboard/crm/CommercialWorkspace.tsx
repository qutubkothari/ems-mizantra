...
export default function CommercialWorkspace({ mode, users, canCreate, canEdit }: { mode: Mode; users: User[]; canCreate: boolean; canEdit: boolean }) {
  ...
  return <section className="space-y-3">
    ...
    {mode === "accounts" && <div className="overflow-x-auto rounded-2xl border bg-white"><table className="min-w-full text-sm"><thead className="border-b bg-[#F7F3EA] text-left text-xs uppercase text-[#6F5A49]"><tr><th className="p-3">Customer</th><th className="p-3">Type</th><th className="p-3">Industry / territory</th><th className="p-3">Contact</th><th className="p-3">Owner</th></tr></thead><tbody>{accounts.map((row) => <tr key={row.id} className="border-b"><td className="p-3"><b>{row.account_name}</b><small className="block text-slate-500">{row.account_number}</small></td><td className="p-3">{row.account_type}</td><td className="p-3">{row.industry || "—"}<small className="block text-slate-500">{row.territory || "—"}</small></td><td className="p-3">{row.email || row.phone || "—"}</td><td className="p-3">{userName(row.owner_user_id)}</td></tr>)}</tbody></table>{!accounts.length && !busy && <Empty/>}</div>}
