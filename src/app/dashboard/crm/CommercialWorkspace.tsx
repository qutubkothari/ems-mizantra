...
export default function CommercialWorkspace({ mode, users, canCreate, canEdit }: { mode: Mode; users: User[]; canCreate: boolean; canEdit: boolean }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(`${mode === "accounts" ? "Customer" : mode === "contacts" ? "Contact" : "Opportunity"} created.`);
      formElement.reset();
      setForm(false);
      await load();
  ...
  const editRecord=async(kind:"accounts"|"contacts"|"opportunities",row:any)=>{const label=kind==="accounts"?"Customer name":kind==="contacts"?"Contact first name":"Opportunity name";const fieldName=kind==="accounts"?"account_name":kind==="contacts"?"first_name":"opportunity_name";const value=window.prompt(label,row[fieldName]);if(!value?.trim())return;try{await apiClient.patch(`/crm/${kind}/${row.id}`,{...row,[fieldName]:value.trim()});setMessage("Record updated.");await load();}catch(next:any){setError(next?.message||"Unable to update the EMS record.");}};
