...
function fullName(contact?: Contact) {
  return contact ? [contact.salutation, contact.first_name, contact.last_name].filter(Boolean).join(" ") : "â€”";
}
...
export default function CommercialWorkspace({ mode, users, canCreate, canEdit }: { mode: Mode; users: User[]; canCreate: boolean; canEdit: boolean }) {
  ...
  return <section className="space-y-4">
    <header className="flex flex-col gap-3 rounded-2xl border border-[#E7DBC5] bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
      <div><p className="text-xs font-bold uppercase tracking-wider text-[#8B6F47]">Commercial EMS</p><h2 className="mt-1 flex items-center gap-2 text-2xl font-black"><Icon className="h-6 w-6"/>{title}</h2><p className="mt-1 text-sm text-[#6F5A49]">{mode === "accounts" ? "Organizations and customers with repeat commercial relationships." : mode === "contacts" ? "Decision-makers and communication consent by account." : "Commercial deals separated from initial lead qualification."}</p></div>
      <div className="flex gap-2"><button onClick={load} className="rounded-xl border p-2" aria-label="Refresh commercial EMS"><RefreshCw className={`h-5 w-5 ${busy ? "animate-spin" : ""}`}/></button>{canCreate && <button onClick={() => setForm(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#3E2A1F] px-4 py-2.5 text-sm font-black text-white"><Plus className="h-4 w-4"/>New {mode === "accounts" ? "account" : mode === "contacts" ? "contact" : "opportunity"}</button>}</div>
    </header>
    ...
    <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${title.toLowerCase()}â€¦`} className={field}/>
    {mode === "accounts" && <div className="overflow-x-auto rounded-2xl border bg-white"><table className="min-w-full text-sm"><thead className="border-b bg-[#F7F3EA] text-left text-xs uppercase text-[#6F5A49]"><tr><th className="p-3">Account</th><th className="p-3">Type</th><th className="p-3">Industry / territory</th><th className="p-3">Contact</th><th className="p-3">Owner</th><th className="p-3">ERP link</th></tr></thead><tbody>{accounts.map((row) => <tr key={row.id} className="border-b"><td className="p-3"><b>{row.account_name}</b><small className="block text-slate-500">{row.account_number}</small></td><td className="p-3">{row.account_type}</td><td className="p-3">{row.industry || "â€”"}<small className="block text-slate-500">{row.territory || "â€”"}</small></td><td className="p-3">{row.email || row.phone || "â€”"}</td><td className="p-3">{userName(row.owner_user_id)}</td><td className="p-3">{row.customer_id ? <span className="font-bold text-emerald-700">Customer linked</span> : "Prospect"}</td></tr>)}</tbody></table>{!accounts.length && !busy && <Empty/>}</div>}
    {mode === "contacts" && <div className="overflow-x-auto rounded-2xl border bg-white"><table className="min-w-full text-sm"><thead className="border-b bg-[#F7F3EA] text-left text-xs uppercase text-[#6F5A49]"><tr><th className="p-3">Contact</th><th className="p-3">Account</th><th className="p-3">Role</th><th className="p-3">Phone / email</th><th className="p-3">Consent</th></tr></thead><tbody>{contacts.map((row) => <tr key={row.id} className="border-b"><td className="p-3"><b>{fullName(row)}</b>{row.is_primary && <small className="ml-2 rounded bg-emerald-50 px-2 py-1 font-bold text-emerald-700">PRIMARY</small>}</td><td className="p-3">{row.account?.account_name || "â€”"}</td><td className="p-3">{row.job_title || "â€”"}</td><td className="p-3">{row.mobile || "â€”"}<small className="block text-slate-500">{row.email || "â€”"}</small></td><td className="p-3 text-xs">Email {row.email_consent}<br/>WhatsApp {row.whatsapp_consent}{row.do_not_call && <b className="block text-red-700">DO NOT CALL</b>}</td></tr>)}</tbody></table>{!contacts.length && !busy && <Empty/>}</div>}
    {mode === "opportunities" && <div className="flex gap-3 overflow-x-auto pb-3">{data.stages.map((stage) => <div key={stage.id} className="w-72 shrink-0 rounded-2xl border bg-white" style={{borderTop:`4px solid ${stage.colour || "#8B6F47"}`}}><div className="border-b p-3"><b>{stage.stage_name}</b><small className="block text-slate-500">{stage.probability}% Â· {opportunities.filter((row) => row.stage_id === stage.id).length} deal(s)</small></div><div className="space-y-2 p-2">{opportunities.filter((row) => row.stage_id === stage.id).map((row) => <article key={row.id} className="rounded-xl border p-3"><b className="text-sm">{row.opportunity_name}</b><small className="block text-slate-500">{row.opportunity_number} Â· {row.account?.account_name}</small><p className="mt-2 font-black">{money(row.amount,row.currency_code)}</p><p className="text-xs text-slate-500">Close {row.expected_close_date || "not set"} Â· {userName(row.owner_user_id)}</p>{row.next_step && <p className="mt-2 rounded bg-[#F7F3EA] p-2 text-xs">Next: {row.next_step}</p>}{canEdit && <select value={row.stage_id} onChange={(event) => changeStage(row,event.target.value)} className={`${field} mt-2 py-1.5`}>{data.stages.map((next) => <option key={next.id} value={next.id}>{next.stage_name}</option>)}</select>}</article>)}{!opportunities.some((row) => row.stage_id === stage.id) && <p className="p-3 text-center text-xs text-slate-400">No deals</p>}</div></div>)}</div>}
