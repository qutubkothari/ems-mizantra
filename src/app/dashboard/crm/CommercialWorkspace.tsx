...
export default function CommercialWorkspace({ mode, users, canCreate, canEdit }: { mode: Mode; users: User[]; canCreate: boolean; canEdit: boolean }) {
  ...
  return <section className="space-y-3">
    <header className="flex flex-col gap-2 rounded-xl border border-[#E7DBC5] bg-white px-4 py-3 shadow-sm md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-2"><Icon className="h-5 w-5 text-[#8B6F47]"/><div><h2 className="text-xl font-black">{title}</h2><p className="text-xs text-[#6F5A49]">{mode === "accounts" ? "Organizations and customers." : mode === "contacts" ? "Decision-makers and consent." : "Active commercial deals."}</p></div></div>
      <div className="flex gap-2"><button onClick={load} className="rounded-lg border p-2" aria-label="Refresh commercial EMS"><RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`}/></button>{canCreate && <button onClick={() => setForm(true)} className="inline-flex items-center gap-2 rounded-lg bg-[#3E2A1F] px-3 py-2 text-sm font-black text-white"><Plus className="h-4 w-4"/>New {mode === "accounts" ? "account" : mode === "contacts" ? "contact" : "opportunity"}</button>}</div>
    </header>
