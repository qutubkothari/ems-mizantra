...
function CrmPageContent() {
  ...
  async function convert() {
    if (
      !selected ||
      !window.confirm(
        "Convert this lead to the customer master? The EMS history will remain linked.",
      )
    )
      return;
    setSaving(true);
  ...
  async function saveIntakeControls(event: FormEvent) {
    ...
    try {
      ...
      setError(err?.message || "Unable to save EMS intake controls.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function permanentlyDeleteIntake(ids: string[]) {
    ...
    if (
      window.prompt(
        `This permanently removes the selected messages from the EMS intake ledger. Any EMS leads already created from them will remain until separately deleted.\n\nType ${confirmation} to continue.`,
      ) !== confirmation
    )
      return;
    setSaving(true);
  ...
  if (permissionRequired && !allowed("view")) {
    return (
      <main className="min-h-screen bg-[#F7F3EA] p-3 md:p-6">
        <ErpWorkspaceState
          mode="restricted"
          title="EMS access is restricted"
          description="Your current role does not include access to EMS records. Ask an administrator to grant the required EMS screen permission."
          action={
            <Link
              href="/dashboard"
        ...
        {isOverview ? <header className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#203A43] via-[#2C5364] to-[#167D7F] p-5 text-white shadow-lg md:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black">Intelligent EMS</h1>
              ...
              <div className="mt-3 space-y-2">
                ...
                {!data?.recommended_actions.length && (
                  <p className="text-sm text-teal-100">
                    No urgent EMS action right now.
                  </p>
                )}
              </div>
            </aside>
